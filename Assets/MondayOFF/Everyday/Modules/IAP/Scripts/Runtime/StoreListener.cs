...
namespace MondayOFF {
    internal class StoreListener : IStoreListener {
        ...
        private Action onInitialized = default;
        ...
        internal void RegisterProducts(string productID, Action onPurchase) {
            ...
            while (productData.purchaseCount > 0) {
                Debug.Log($"[EVERYDAY] {productData.purchaseCount} puchases were made before initialization. Process them");
                onPurchase.Invoke();
                --productData.purchaseCount;
            }
        }

        internal bool PurchaseProduct(string productID) {
            if (!products.TryGetValue(productID, out var productData)) {
                Debug.LogWarning($"[EVERYDAY] {productID} does not exist in EverydayProducts");
                return false;
            }
        ...
        internal StoreListener(Dictionary<string, ProductData> productList) {
